...
export default function DaroUsers() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      addToast(editingId ? 'DARO User updated successfully' : 'DARO User added successfully', 'success');
      closeModal();
      queryClient.invalidateQueries({ queryKey: ['daro'] });
    },
    onError: (err) => {
      console.error(err);
      addToast(err.response?.data?.error || 'Failed to save DARO User', 'error');
    }
  });
  ...
  const handleToggleStatus = (v) => {
    ...
    if (!window.confirm(`Are you sure you want to ${newStatus ? 'activate' : 'stop'} this DARO User's account?`)) return;
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => {
      console.error(err);
      addToast(err.response?.data?.error || 'Failed to delete DARO User', 'error');
    }
  });

  const handleDelete = (id) => {
    if (!window.confirm('Are you sure you want to delete this DARO User? This action cannot be undone.')) return;
    deleteMutation.mutate(id);
  };

  const filtereddaroUsers = useMemo(() => {
    return daroUsers.filter(v => {
      if (search) {
  ...
  const pagination = usePagination(filtereddaroUsers, 12);
  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-12 pt-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">DARO Users</h1>
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-slate-800">No DARO Users found</h3>
            <p className="text-slate-500 mt-1">When DARO Users register or are added, they will appear here.</p>
          </div>
        ) : (
          <>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900">{editingId ? 'Edit DARO User' : 'Add New DARO User'}</h2>
            </div>
