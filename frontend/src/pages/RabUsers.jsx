...
export default function RabUsers() {
  ...
  const { data: rabUsers = [], isLoading: loading } = useQuery({
    queryKey: ['rab-users'],
    queryFn: async () => {
      const res = await axios.get('/rvf-api/auth/rab-users');
      return res.data;
    },
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editingId) {
        return axios.put(`/rvf-api/auth/rab-users/${editingId}`, formData);
      } else {
        return axios.post('/rvf-api/auth/rab-users', formData);
      }
    },
    onSuccess: () => {
      addToast(editingId ? 'RAB User updated successfully' : 'RAB User added successfully', 'success');
      closeModal();
      queryClient.invalidateQueries({ queryKey: ['rab-users'] });
    },
    onError: (err) => {
      console.error(err);
      addToast(err.response?.data?.error || 'Failed to save RAB User', 'error');
    }
  });

  const toggleStatusMutation = useMutation({
    mutationFn: async ({ id, is_active }) => axios.put(`/rvf-api/auth/rab-users/${id}`, { is_active }),
    onSuccess: (_, variables) => {
      addToast(`RAB User account ${variables.is_active ? 'activated' : 'stopped'} successfully`, 'success');
      queryClient.invalidateQueries({ queryKey: ['rab-users'] });
    },
    onError: (err) => {
      console.error(err);
  ...
  const handleToggleStatus = (v) => {
    ...
    if (!window.confirm(`Are you sure you want to ${newStatus ? 'activate' : 'stop'} this RAB User's account?`)) return;
    toggleStatusMutation.mutate({ id: v.id, is_active: newStatus });
  };

  const deleteMutation = useMutation({
    mutationFn: async (id) => axios.delete(`/rvf-api/auth/rab-users/${id}`),
    onSuccess: () => {
      addToast('RAB User deleted successfully', 'success');
      queryClient.invalidateQueries({ queryKey: ['rab-users'] });
    },
    onError: (err) => {
      console.error(err);
      addToast(err.response?.data?.error || 'Failed to delete RAB User', 'error');
    }
  });

  const handleDelete = (id) => {
    if (!window.confirm('Are you sure you want to delete this RAB User? This action cannot be undone.')) return;
    deleteMutation.mutate(id);
  };

  const filteredrabUsers = useMemo(() => {
    return rabUsers.filter(v => {
      if (search) {
  ...
  const pagination = usePagination(filteredrabUsers, 12);
  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-12 pt-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">RAB Users</h1>
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-slate-800">No RAB Users found</h3>
            <p className="text-slate-500 mt-1">When RAB Users register or are added, they will appear here.</p>
          </div>
        ) : (
          <>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900">{editingId ? 'Edit RAB User' : 'Add New RAB User'}</h2>
            </div>
