...
export default function RabAuth({ mode = 'login' }) {
  ...
  const loginMutation = useMutation({
    mutationFn: async (payload) => {
      const endpoint = mode === 'register' ? '/rvf-api/rab/register' : '/rvf-api/rab/login';
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-[450px] p-8 pt-10 relative overflow-hidden text-left border border-slate-200">
        ...
        <div className="mb-6">
          ...
          <p className="text-[16px] text-[#373A3C] leading-normal">
            Enter your phone number to access approved spraying forms.
          </p>
        </div>
        ...
        <form onSubmit={handleVerifyPhone}>
          ...
          <div className="text-center text-[14px] text-[#373A3C]">
            {mode === 'login' ? (
              <>Don't have an account? <button type="button" onClick={() => navigate('/rab-signup')} className="text-[#0b57d0] font-medium hover:underline focus:outline-none">Register here</button></>
            ) : (
              <>Already have an account? <button type="button" onClick={() => navigate('/rab-login')} className="text-[#0b57d0] font-medium hover:underline focus:outline-none">Login here</button></>
            )}
          </div>
        </form>
